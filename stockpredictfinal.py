...
def loadDataset():
    ...
    apple = pd.read_csv("./Yahoo-Finance-Dataset/Apple_YahooDataSet.csv")
    dataFrame = apple.drop('Date',axis=1)
    ...
    applec = pd.read_csv("./Yahoo-Finance-Dataset/competitor_YahooDataSet.csv")
    dfcomp = applec.drop('Date',axis=1)
...
def distKNN():
    ...
    text.insert(END, "Accuracy of KNN with Dist weights : "+str(distknn*100)+"\n\n")
def predModel():
    text.delete('1.0', END)
